...
export default function UserManagementDashboard() {
  ...
  const createUser = async () => {
    try {
      ...
      const payload = {
        ...form,
        subjects: form.role === "TEACHER" ? parseCsv(form.subjects) : [],
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: 10, marginBottom: 12 }}>
        ...
        <StatCard label="Active" value={summary?.activeUsers || 0} />
        <StatCard label="Inactive" value={summary?.inactiveUsers || 0} />
        <StatCard label="Suspended" value={summary?.suspendedUsers || 0} />
        <StatCard label="Recent Logins" value={summary?.recentLoginsToday || 0} />
      </div>

      <div style={{ border: "1px solid #d8e2ef", borderRadius: 10, padding: 12, marginBottom: 12, background: "#fff" }}>
        <h3 style={{ marginTop: 0 }}>Filters and Bulk Actions</h3>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button onClick={applyBulkAction} disabled={busy || selectedIds.length === 0}>
