...
function SessionsPanel({ sessions, sessionForm, setSessionForm, createSession, onSessionStatusChange }) {
  return (
    <section style={panelStyle}>
      <h3 style={{ marginTop: 0 }}>Admission Sessions</h3>
      <div style={{ display: "grid", gap: 6, marginBottom: 10 }}>
        {sessions.map((row) => (
          <div key={row.id} style={{ border: "1px solid #e2eaf5", borderRadius: 8, padding: 8, background: "#fff" }}>
            <strong>{row.title}</strong>
            <div style={{ fontSize: 13, color: "#4d647f" }}>{row.startDate} - {row.endDate || "Open"}</div>
      ...
      <h4 style={{ marginBottom: 6 }}>Create Session</h4>
      <Grid2>
        ...
        <input placeholder="Academic Session" value={sessionForm.academicSession} onChange={(e) => setSessionForm((p) => ({ ...p, academicSession: e.target.value }))} />
