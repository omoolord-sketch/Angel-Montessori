...
function getLmsBaseByRole(role) {
  ...
  if (role === "TEACHER") return "/teacher/lms";
...
export default function LMSDashboard() {
  ...
  const renderSetup = () => (
    <div style={{ display: "grid", gap: 12 }}>
      <div style={card}>
        <h3 style={{ marginTop: 0 }}>Academic Setup</h3>
        <div style={grid2}>
          <input placeholder="Session name (e.g. 2025/2026)" value={sessionForm.sessionName} onChange={(e) => setSessionForm({ ...sessionForm, sessionName: e.target.value })} />
          <label style={{ display: "flex", alignItems: "center", gap: 6 }}><input type="checkbox" checked={sessionForm.isActive} onChange={(e) => setSessionForm({ ...sessionForm, isActive: e.target.checked })} /> Active</label>
        ...
        <div style={{ marginTop: 10, display: "grid", gap: 6 }}>
          {(sessionOptions || []).map((x) => (
            <div key={x.id} style={{ display: "flex", justifyContent: "space-between", borderTop: "1px dashed #dbe6f4", paddingTop: 6 }}>
              <span>{x.sessionName} {x.isActive ? "(Active)" : ""}</span>
              {!x.isActive ? <button onClick={() => activateSession(x.id)}>Set Active</button> : null}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={card}>
        ...
        <div style={grid2}>
          ...
          <input placeholder="Term name" value={termForm.termName} onChange={(e) => setTermForm({ ...termForm, termName: e.target.value })} />
          ...
          <label style={{ display: "flex", alignItems: "center", gap: 6 }}><input type="checkbox" checked={termForm.isActive} onChange={(e) => setTermForm({ ...termForm, isActive: e.target.checked })} /> Active</label>
        ...
        <div style={{ marginTop: 10, display: "grid", gap: 6 }}>
          {termOptions.map((x) => (
            <div key={x.id} style={{ display: "flex", justifyContent: "space-between", borderTop: "1px dashed #dbe6f4", paddingTop: 6 }}>
              <span>{x.termName} {x.isActive ? "(Active)" : ""}</span>
              {!x.isActive ? <button onClick={() => activateTerm(x.id)}>Set Active</button> : null}
            </div>
          ))}
        </div>
      </div>
    ...
    <div style={{ display: "grid", gap: 12 }}>
      {(role === "ADMIN" || role === "TEACHER") ? (
        <div style={card}>
          ...
          {role === "ADMIN" ? <button style={{ marginTop: 8 }} onClick={seedMappings}>Seed Templates by Class</button> : null}
        </div>
      ) : null}
