...
export default function TeacherAnalyticsDashboard() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f3f7fc", padding: 16 }}>
      <div style={{ maxWidth: 1300, margin: "0 auto", display: "grid", gap: 12 }}>
        <div style={card}>
          <h2 style={{ marginTop: 0 }}>Teacher Performance and Support</h2>
          <p style={{ marginBottom: 8, color: "#4b637f" }}>
            {isTeacher
          ...
          <div style={{ display: "grid", gap: 8, gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))" }}>
            <select
              ...
              onChange={(e) => setFilters((prev) => ({ ...prev, sessionId: e.target.value, termId: "" }))}
            >
              {(metadata.sessions || []).map((row) => (
                <option key={row.id} value={row.id}>{row.sessionName}</option>
              ))}
            </select>

            <select value={activeTermId} onChange={(e) => setFilters((prev) => ({ ...prev, termId: e.target.value }))}>
              {(metadata.terms || [])
                .filter((row) => !activeSessionId || String(row.sessionId) === String(activeSessionId))
                .map((row) => (
                  <option key={row.id} value={row.id}>{row.termName}</option>
                ))}
            </select>
            ...
            {!isTeacher && (
              <>
                ...
                <select
                  ...
                  onChange={(e) => setFilters((prev) => ({ ...prev, flagStatus: e.target.value }))}
                >
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 8 }}>
            <button onClick={() => refresh(selectedTeacherId)} disabled={loading}>Refresh</button>
            {!isTeacher && <button onClick={runSnapshot} disabled={loading}>Run Snapshot Job</button>}
            {!isTeacher && <button onClick={runFlags} disabled={loading}>Run Flags Job</button>}
            {!isTeacher && <button onClick={() => downloadExport("teachers", "csv")} disabled={loading}>Teachers CSV</button>}
            {!isTeacher && <button onClick={() => downloadExport("teachers", "pdf")} disabled={loading}>Teachers PDF</button>}
            {!isTeacher && <button onClick={() => downloadExport("flags", "csv")} disabled={loading}>Flags CSV</button>}
            {!isTeacher && <button onClick={() => downloadExport("flags", "pdf")} disabled={loading}>Flags PDF</button>}
            {!isTeacher && <button onClick={() => downloadExport("teacher", "csv")} disabled={loading || !selectedTeacherId}>Selected CSV</button>}
            {!isTeacher && <button onClick={() => downloadExport("teacher", "pdf")} disabled={loading || !selectedTeacherId}>Selected PDF</button>}
            {isTeacher && <button onClick={() => downloadExport("teacher", "csv")} disabled={loading}>My CSV</button>}
            {isTeacher && <button onClick={() => downloadExport("teacher", "pdf")} disabled={loading}>My PDF</button>}
            <button onClick={printProfile} disabled={!teacherDetail}>Print Profile</button>
          </div>

          {loading && <p style={{ marginTop: 10 }}>Loading teacher analytics...</p>}
