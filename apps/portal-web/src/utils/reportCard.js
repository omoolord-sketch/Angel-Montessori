...
function componentScore(row = {}, keys = []) {
...
function computeTermStats(rows = []) {
  const total = rows.reduce((sum, row) => sum + scoreValue(row), 0);
...
function getRowsForTerm(results = [], session, term) {
  return results
    .filter((row) => String(row.session || row.sessionName || "") === String(session) && String(row.term || row.termName || "") === String(term))
    .sort((a, b) => String(a.subject || "").localeCompare(String(b.subject || "")));
}

function getCumulativeRows(results = [], session, uptoTerm) {
  const upto = getTermIndex(uptoTerm);
  return results.filter((row) => String(row.session || row.sessionName || "") === String(session) && getTermIndex(row.term || row.termName) <= upto);
}
...
export function buildReportCardHtml({ student, results, summaries, reports, promotionDecisions = [], selectedTerm, schoolName, schoolAddress, logoUrl }) {
  ...
  const summaryMeta =
    summaries.find((r) => String(r.session) === String(activeTerm.session) && String(r.term) === String(activeTerm.term)) ||
    {};
  ...
  const promotion = showPromotion
    ...
    : null;
  const firstTermStats = showPromotion ? computeTermStats(getRowsForTerm(results, activeTerm.session, "First Term")) : null;
  const secondTermStats = showPromotion ? computeTermStats(getRowsForTerm(results, activeTerm.session, "Second Term")) : null;
  const principalComment = reportMeta.principalComment || reportMeta.headTeacherComment || "-";
  const nextClassRecommendation = reportMeta.nextClassRecommendation || reportMeta.nextClassName || promotion?.nextClassName || "-";

  const rowsHtml = termResults
    .map((row, idx) => {
      const total = scoreValue(row);
      const grade = row.grade || row.gradeLetter || gradeFromScore(total);
      const remark = row.teacherRemark || row.remark || row.comment || remarkFromScore(total);
      const position = row.positionInSubject || row.subjectPosition || row.position || "-";
      return `
      <tr>
        <td>${idx + 1}</td>
        <td>${escapeHtml(row.subject || row.subjectName || "-")}</td>
        <td>${escapeHtml(formatScore(componentScore(row, ["ca1", "CA1"])))}</td>
        <td>${escapeHtml(formatScore(componentScore(row, ["ca2", "CA2"])))}</td>
        <td>${escapeHtml(formatScore(componentScore(row, ["ca3", "CA3"])))}</td>
        <td>${escapeHtml(formatScore(componentScore(row, ["examScore", "exam", "exam_score"])))}</td>
        <td>${escapeHtml(formatScore(total))}</td>
  ...
  <div class="sheet">
    ...
    <div class="section">
      ...
      <table>
        ...
        <tbody>
          ${rowsHtml || '<tr><td colspan="10">No approved result records found for this term.</td></tr>'}
          ${showCumulative ? `
          <tr class="cum-row">
            <td colspan="6">${escapeHtml(cumulativeRowLabel)}</td>
            <td>${escapeHtml(formatScore(cumulativeStats.total))}</td>
      ...
      <div class="summary">
        <div class="card"><strong>Term Total Score:</strong> ${escapeHtml(formatScore(termStats.total))}</div>
        ...
        <div class="card"><strong>Class Average:</strong> ${escapeHtml(summaryMeta.classAverage ?? reportMeta.classAverage ?? "-")}</div>
      </div>

      ${showCumulative ? `
      <div class="summary">
        <div class="card"><strong>Cumulated Score:</strong> ${escapeHtml(formatScore(cumulativeStats.total))}</div>
      ...
      ${showPromotion ? `
      <div class="summary">
        <div class="card"><strong>First Term Average:</strong> ${escapeHtml(firstTermStats.average.toFixed(2))}</div>
        <div class="card"><strong>Second Term Average:</strong> ${escapeHtml(secondTermStats.average.toFixed(2))}</div>
        <div class="card"><strong>Third Term Score:</strong> ${escapeHtml(formatScore(termStats.total))}</div>
        <div class="card"><strong>Annual Cumulative Average:</strong> ${escapeHtml(cumulativeStats.average.toFixed(2))}</div>
        <div class="card"><strong>Promotion Status:</strong> ${escapeHtml(promotion.decision)}</div>
        <div class="card"><strong>Next Class Recommendation:</strong> ${escapeHtml(nextClassRecommendation)}</div>
      </div>` : ""}
      ...
      <div class="comments">
        ...
        <div><strong>Principal's Comment:</strong> ${escapeHtml(principalComment)}</div>
        ${showPromotion ? `<div><strong>Next Class Recommendation:</strong> ${escapeHtml(nextClassRecommendation)}</div>` : ""}
