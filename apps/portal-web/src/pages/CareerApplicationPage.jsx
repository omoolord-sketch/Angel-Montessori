...
const MAX_CAREER_UPLOAD_SIZE_BYTES = 30 * 1024;
const UPLOAD_SIZE_HINT = "Maximum 30KB per file.";
...
export default function CareerApplicationPage() {
  ...
  const handleFile = async (event, key, multi = false) => {
    ...
    const oversized = files.find((file) => file.size > MAX_CAREER_UPLOAD_SIZE_BYTES);
    if (oversized) {
      event.target.value = "";
      setForm((prev) => ({
        ...prev,
      ...
      setError(`${oversized.name} is too large. ${UPLOAD_SIZE_HINT}`);
      return;
    }
    try {
              ...
              <form onSubmit={handleSubmit} style={{ display: "grid", gap: 14 }}>
                ...
                <div className="public-grid-3">
                  <label style={labelStyle}>Upload CV
                    <small style={uploadHintStyle}>{UPLOAD_SIZE_HINT}</small>
