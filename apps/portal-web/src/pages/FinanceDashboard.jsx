...
export default function FinanceDashboard() {
  ...
  const [bulkPaymentForm, setBulkPaymentForm] = useState(defaultBulkPaymentForm);
  const [bulkPaymentRows, setBulkPaymentRows] = useState([]);
  ...
  const loadBulkPaymentClass = async (classId, preset = {}) => {
    setBulkPaymentForm((prev) => ({ ...prev, classId }));
    if (!classId) {
      setBulkPaymentRows([]);
      return;
    }
    ...
    try {
      setBusy("bulk-class");
      setError("");
      const response = await getFinanceStudentFees({
        sessionId: feeFilters.sessionId || setup?.activeSessionId,
        termId: feeFilters.termId || setup?.activeTermId,
        classId,
      });
      const records = Array.isArray(response.data?.records) ? response.data.records : [];
      setBulkPaymentRows(records.map((item) => {
        const isPreset = String(item.studentId) === String(preset.studentId || "");
        const presetAmount = isPreset ? Number(preset.amount ?? item.balance) : 0;
        return {
          studentId: item.studentId,
          studentName: item.studentName,
          admissionNumber: students.find((student) => String(student.id) === String(item.studentId))?.admissionNumber || "",
          className: item.className,
          balance: Number(item.balance || 0),
          selected: isPreset && presetAmount > 0,
          amount: isPreset && presetAmount > 0 ? String(presetAmount) : "",
          reference: "",
        };
      }));
    } catch (err) {
      setBulkPaymentRows([]);
      setError(getErrorMessage(err, "Failed to load students for this class"));
    } finally {
      setBusy("");
    }
  };
  ...
  const primePaymentForm = (row) => {
    ...
    loadBulkPaymentClass(row.classId, {
      studentId: row.studentId,
      amount: row.balance,
    });
  };

  const updateBulkPaymentRow = (studentId, patch) => {
    setBulkPaymentRows((rows) => rows.map((row) => (
  ...
  const selectedBulkPayments = useMemo(
    () => bulkPaymentRows.filter((row) => row.selected && Number(row.amount || 0) > 0),
    [bulkPaymentRows]
  );

  const bulkPaymentTotal = useMemo(
    () => selectedBulkPayments.reduce((sum, row) => sum + Number(row.amount || 0), 0),
    [selectedBulkPayments]
  );
    ...
    { key: "paymentMethod", label: "Method" },
    {
      ...
      render: (row) => (
        <div className="finance-inline-actions">
          <button type="button" onClick={() => handlePrintReceipt(row.id)}>Print Receipt</button>
          <button
            type="button"
            className="danger"
            disabled={String(row.status || "").toLowerCase() === "voided"}
            onClick={() => runAction("void-payment", () => voidFinancePayment(row.id, { reason: "Payment reversed from finance desk" }), "Payment reversed and student balance recalculated.")}
          >
            {String(row.status || "").toLowerCase() === "voided" ? "Voided" : "Void"}
          </button>
        </div>
      ),
    },
      ...
      <div className="portal-surface-shell finance-shell">
        <section className="portal-surface-hero finance-hero">
          ...
          <div className="portal-surface-actions">
            ...
            <Link to="/dashboard/payments" className="portal-surface-action secondary">School Fees</Link>
        ...
        <div className="finance-quick-actions">
          <Link to="/dashboard/payments" className="finance-quick-link">School Fees</Link>
        ...
        {!loading && activeTab === "receive" ? (
          <div className="finance-card">
            <div className="finance-card-header">
              <div>
                <h2>Receive Payments by Class</h2>
                <p>Select a class, enter payments for several students, and post the full batch at once.</p>
              </div>
            </div>
            <form className="finance-bulk-payment-form" onSubmit={(event) => {
              event.preventDefault();
              const payments = selectedBulkPayments.map((row, index) => ({
                studentId: row.studentId,
                amount: Number(row.amount || 0),
                paymentMethod: bulkPaymentForm.paymentMethod,
                reference: row.reference || (
                  bulkPaymentForm.reference
                    ? `${bulkPaymentForm.reference}-${String(index + 1).padStart(2, "0")}`
                    : ""
                ),
                description: bulkPaymentForm.description
                  ? `${bulkPaymentForm.description} - ${row.studentName}`
                  : `School fee payment - ${row.studentName}`,
              }));

              runAction("bulk-payment", async () => {
                if (!bulkPaymentForm.classId) {
                  throw new Error("Select a class before entering payments");
                }
                if (!payments.length) {
                  throw new Error("Select at least one student and enter a payment amount");
                }
                await receiveFinancePaymentsBulk({
                  sessionId: feeFilters.sessionId || setup?.activeSessionId,
                  termId: feeFilters.termId || setup?.activeTermId,
                  paymentMethod: bulkPaymentForm.paymentMethod,
                  payments,
                });
                setBulkPaymentForm(defaultBulkPaymentForm);
                setBulkPaymentRows([]);
              }, `${payments.length} payment${payments.length === 1 ? "" : "s"} received and receipts created.`, "fees");
            }}>
              <div className="finance-form-grid finance-bulk-payment-controls">
                <label>
                  Class
                  <select
                    value={bulkPaymentForm.classId}
                    onChange={(event) => loadBulkPaymentClass(event.target.value)}
                    disabled={busy === "bulk-class" || busy === "bulk-payment"}
                  >
                ...
                <label>
                  Payment Method
                  <select value={bulkPaymentForm.paymentMethod} onChange={(e) => setBulkPaymentForm((prev) => ({ ...prev, paymentMethod: e.target.value }))}>
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) => setBulkPaymentForm((prev) => ({ ...prev, reference: e.target.value }))}
              ...
              {bulkPaymentForm.classId ? (
                <>
                  <div className="finance-bulk-toolbar">
                    <div>
                      <strong>{bulkPaymentRows.length} students</strong>
                      <span>{selectedBulkPayments.length} selected | {formatCurrency(bulkPaymentTotal, currency)}</span>
                    </div>
                    <div className="finance-form-actions">
                      <button type="button" className="secondary" onClick={selectAllOutstanding}>Fill Outstanding</button>
                      <button type="button" className="secondary" onClick={clearBulkPayments}>Clear</button>
                    </div>
                  </div>

                  <div className="finance-table-shell finance-bulk-table-shell">
                    <table className="finance-table finance-bulk-table">
                      <thead>
                        <tr>
                          <th>Pay</th>
                          <th>Student</th>
                      ...
                      <tbody>
                        {bulkPaymentRows.length ? bulkPaymentRows.map((row) => (
                          <tr key={row.studentId}>
                            <td>
                              <input
                                ...
                                onChange={(event) => updateBulkPaymentRow(row.studentId, {
                                  selected: event.target.checked,
                                  amount: event.target.checked && !row.amount && row.balance > 0 ? String(row.balance) : row.amount,
                                })}
                              />
                            </td>
                            <td>
                              <strong>{row.studentName}</strong>
                              <span className="finance-bulk-class-name">{row.className}</span>
                            </td>
                            ...
                            <td>
                              <input
                                ...
                                onChange={(event) => updateBulkPaymentRow(row.studentId, { reference: event.target.value })}
                              />
                            </td>
                          </tr>
                        )) : (
                          <tr>
                            <td colSpan="6" className="finance-table-empty">
                              {busy === "bulk-class" ? "Loading class students..." : "No active students found in this class."}
                            </td>
                          </tr>
                        )}
                      </tbody>
                ...
                <div className="finance-empty-state">
                  ...
                  <span>The complete class list and current balances will appear here.</span>
                </div>
              )}

              <div className="finance-form-actions finance-bulk-submit">
                <button type="submit" disabled={busy === "bulk-payment" || !selectedBulkPayments.length}>
                  {busy === "bulk-payment" ? "Posting Payments..." : `Post ${selectedBulkPayments.length || ""} Payment${selectedBulkPayments.length === 1 ? "" : "s"}`}
                </button>
              </div>
            </form>
          </div>
        ...
        {!loading && activeTab === "settings" ? (
          <div className="finance-card">
            <div className="finance-card-header"><div><h2>Finance Settings</h2><p>Control payment methods, categories, and the school-wide active academic scope.</p></div></div>
            <form className="finance-form-grid" onSubmit={(event) => {
              event.preventDefault();
